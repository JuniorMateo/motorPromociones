...
import claro.com.pe.evalua.bono.recalculo.handler.BssAgreementManagementClientHandler;
		 ...
		 try {
				...
				if ( e.toString().contains( SocketTimeoutException.class.toString().replace( "class",Constantes.CONSTANTE_VACIA).trim() ) ) {
					LOGGER.error(mensajeTransaccion + "Error producido por timeout en el Servicio "+ propiedades.bssAgreementManagementURL);
					 String msjErr = propiedades.MSJ_IDT3.replace("$ws", Constantes.INTCOP0223).replace("$metodo", Constantes.RETRIEVESUBSCRIPTIONS);
				...
				String msjError = propiedades.MSJ_IDT4.replace("$ws", Constantes.INTCOP0223).replace("$metodo", Constantes.RETRIEVESUBSCRIPTIONS);
